...
// int degrees[100] = {0};
        ...
        //为了获得改变节点的数量：需要新增totalChangeElements变量
...
//为了获得改变节点的数量： level <= currlevel && ifChange == false
            for (unsigned level = 0; level <= currlevel; ++level) {
                ...
                //为了获得改变节点的数量：k < friendQty && ifChange == false
                for (unsigned k = 0; k < friendQty; ++k) {
                    IdType friendId = friends[k]->id_;  //一层里面朋友的id。
                    //把它打印出来看看？（朋友的id）
                    //LOG(LIB_INFO) << friendId <<",";  //为了获得改变节点的数量
